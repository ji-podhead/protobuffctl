...
class Filewatcher {
 ...
 /**
  * A class responsible for generating Protobuf files for various programming languages.
  * It provides a method to generate Protobuf files based on the specified language,
  * the path to the Proto file, and the desired output path.
  */
 class ProtobufGenerator {
     ...
     * @param {string} protoFilePath - The path to the Proto file that should be used as the source.
     ...
     */
    generateProtobuf(language, protoFilePath, outputPath) {
        let command;
        switch (language) {
            case 'go':
                // Example for Go
                command = `protoc --go_out=${outputPath} ${protoFilePath}`;
                break;
            case 'java':
                // Example for Java
                command = `protoc --java_out=${outputPath} ${protoFilePath}`;
                break;
            case 'python':
                // Example for Python
                command = `protoc --python_out=${outputPath} ${protoFilePath}`;
                break;
            case 'csharp':
                // Example for C#
                command = `protoc --objc_out=${outputPath} ${protoFilePath}`;
                break;
            case 'ruby':
                // Example for Ruby
                command = `protoc --ruby_out=${outputPath} ${protoFilePath}`;
                break;
            case 'objc':
            ...
            case 'php':
                // Example for PHP
                command = `protoc --php_out=${outputPath} ${protoFilePath}`;
                break;
            case 'dart':
                // Example for Dart
                command = `protoc --dart_out=${outputPath} ${protoFilePath}`;
                break;
            case 'rust':
                // Example for Rust
                command = `protoc --rust_out=${outputPath} ${protoFilePath}`;
                break;
            case 'swift':
                // Example for Swift
                command = `protoc --swift_out=${outputPath} ${protoFilePath}`;
                break;
            case 'kotlin':
                // Example for Kotlin
                command = `protoc --kotlin_out=${outputPath} ${protoFilePath}`;
                break;
            case 'scala':
                // Example for Scala
                command = `protoc --scala_out=${outputPath} ${protoFilePath}`;
                break;
            case 'js':
                // Example for JavaScript/TypeScript
                const protocGenTsPath = `${process.cwd()}/frontend/node_modules/ts-protoc-gen/bin/protoc-gen-ts`;
                const outDir = `${process.cwd()}/frontend/src`;
                const protoPath = `${process.cwd()}/protos/`;
                command = `
            protoc --plugin="protoc-gen-ts=${protocGenTsPath}" --ts_out="service=grpc-node,mode=grpc-js:${outDir}" --js_out="import_style=commonjs,binary:${outDir}" --proto_path="${protoPath}" ${protoFilePath}
            protoc --plugin="protoc-gen-ts=${protocGenTsPath}" --ts_out="service=grpc-web:${outDir}" --js_out="import_style=commonjs,binary:${outDir}" --proto_path="${protoPath}" ${protoFilePath}
            `;
                break;
            default:
